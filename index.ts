async function processFrame(framePath: string) {
    try {
        ...
        if (people.length > 0) {
            logger.info('Detected people:', 
                people.map(p => `Person at (${p.x},${p.y}): ${(p.confidence * 100).toFixed(1)}% confidence`));
        }
        ...
        for (const vehicle of vehicles) {
            try {
                ...
                if (plates.length > 0) {
                    logger.info('Detected licence plates:', 
                        plates.map(p => `${p.text} (${(p.confidence * 100).toFixed(1)}% confidence)`));
                    plateDetected = true;
                }
    ...
    const color = type === 'people' ? 'green' : 'red';
    ...
    const svgBuffer = Buffer.from(`
        <svg width="${width}" height="${height}">
            ${regions.map(r => `
                ...
                <text x="${r.x}" y="${r.y - 5}" fill="${color}" font-size="16">
    ...
    await original
        ...
        .toFile(path.join(TEMP_DIR, `detected_${type}_${Date.now()}.jpg`));
}function calculateConfidence(data: Buffer, width: number, region: Region): number {
    // Calculate confidence based on edge density and contrast
    let edgeCount = 0;
    let totalPixels = 0;
    ...
    for (let y = region.y; y < region.y + region.height; y++) {
        for (let x = region.x; x < region.x + region.width; x++) {
            ...
            if (data[pos] === 0) {
                ...
                const hasEdge = checkEdges(data, width, x, y);
    ...
    return Math.min(1, (edgeCount / totalPixels) * 3); // Scale factor of 3 to normalize confidence
}

function checkEdges(data: Buffer, width: number, x: number, y: number): boolean {
    const neighbors = [
        [x-1, y], [x+1, y],
        [x, y-1], [x, y+1]
    ];
